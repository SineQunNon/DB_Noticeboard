...
app.get('/getpagetotal', (req, res) => {
    const sql = 'SELECT count(*) AS total FROM POST'

    db.query(sql, (err, result)=>{
        if(err){
            console.err("전체 페이지 개수 조회 실패")
            res.status(501).json({ success: false, message: '서버 오류' })
            return
        }
        const page_num = parseInt(result[0].total) // 좋아요 수 가져오기
        console.log("서버 : ",page_num )
        res.json({total : page_num})
    })
})
